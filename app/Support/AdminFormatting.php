...
class AdminFormatting
{
    ...
    public static function reviewIssueLabel(string $code): string
    {
        return match ($code) {
            ...
            'auto_assigned_maintenance' => 'Unwell pet auto-assigned the maintenance plan — please confirm',
    ...
    public static function reviewIssueExplanation(string $code): string
    {
        return match ($code) {
            ...
            'bad_score_enum' => 'One of the health-insight scores holds an unexpected value (the scores are normally computed automatically), so it may display oddly. Open the Health Scores section and set it to one of the offered band values before publishing.',
            ...
            'auto_assigned_maintenance' => "This pet's results show an imbalance but fired no specific product trigger, so the system auto-assigned the fallback plan (Maintain & Protect). Nothing is wrong — this is the configured behaviour — but as a borderline case it's worth a quick confirm that maintenance is the right plan before publishing. Mark as reviewed once you're happy.",
