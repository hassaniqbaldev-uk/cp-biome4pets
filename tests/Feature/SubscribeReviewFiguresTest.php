...
/**
 * #5 — the review rating/count on the subscribe interstitial are now admin-editable
 * Settings (formerly hardcoded ReportController constants). The controller reads
 * them from the settings table, falling back to the defaults so the page is never
 * blank if unset.
 */
class SubscribeReviewFiguresTest extends TestCase
{
    ...
    public function test_subscribe_falls_back_to_default_review_figures_when_unset(): void
    {
        $report = $this->makeReport();

        $this->get('/report/'.$report->public_token.'/subscribe')
            ->assertOk()
            ->assertSee('4.9')        // Setting::REVIEW_RATING_DEFAULT
            ->assertSee('1,000+');    // Setting::REVIEW_COUNT_DEFAULT
    }

    public function test_subscribe_reads_edited_review_figures_from_settings(): void
    {
